...
// 散列函数
function HashString(HashClass: THashClass; const Source: string; Digest: Pointer = nil): string; 
function HashBuffer(HashClass: THashClass; const Buffer; DataSize: Integer; Digest: Pointer = nil): string;
function HashStream(HashClass: THashClass; Stream: TStream; Digest: Pointer = nil): string;
function HashFile(HashClass: THashClass; const FileName: string; Digest: Pointer = nil): string;
...
function GetTestVector: PChar; register; forward;
procedure RaiseCipherException(const ErrorCode: Integer; const Msg: string); forward;
...
//-----------------------------------------------------------------------------
function HashString(HashClass: THashClass; const Source: string;
...
//   散列后的字符串(对散列值进行Base64编码，不含#0字符)
//-----------------------------------------------------------------------------
function HashStream(HashClass: THashClass; Stream: TStream; Digest: Pointer): string;
begin
...
//   散列后的字符串(对散列值进行Base64编码，不含#0字符)
//-----------------------------------------------------------------------------
function HashFile(HashClass: THashClass; const FileName: string; Digest: Pointer): string;
begin
...
//   散列后的字符串(24字节)
//-----------------------------------------------------------------------------
function HashMD5(const Source: string; Digest: Pointer): string;
begin
...
//   散列后的字符串(28字节)
//-----------------------------------------------------------------------------
function HashSHA(const Source: string; Digest: Pointer): string;
begin
...
//   散列后的字符串(28字节)
//-----------------------------------------------------------------------------
function HashSHA1(const Source: string; Digest: Pointer): string;
begin
...
begin
  ...
  if Size > KeySize then
    if not CheckEncryptKeySize then Size := KeySize
      else RaiseCipherException(CipherErrInvalidKeySize, Format(SInvalidKeySize, [ClassName, 0, KeySize]));
end;
...
begin
  if not Initialized then
    RaiseCipherException(CipherErrNotInitialized, Format(SNotInitialized, [ClassName]));
  S := @Source;
  D := @Dest;
  case FMode of
    emECB:
...
initialization
  ...
  InitTestIsOk := CRC32(CRC32($29524828, PChar(@CRC32) + 41, 1076), GetTestVector, 32) = $78D28741;
