...
// 描述: 接收数据
//-----------------------------------------------------------------------------
int CUdpSocket::RecvBuffer(void *pBuffer, int nSize, CPeerAddress& RemoteAddr)
{
    SockAddr Addr;
    int nBytes;
    socklen_t nSockLen = sizeof(Addr);

    ZeroBuffer(&Addr, sizeof(Addr));
...
// 描述: 发送数据
//-----------------------------------------------------------------------------
int CUdpSocket::SendBuffer(void *pBuffer, int nSize, const CPeerAddress& RemoteAddr,
    int nSendTimes)
{
    int nResult = 0;
    SockAddr Addr;
    ...
    GetSocketAddr(Addr, RemoteAddr.nIp, RemoteAddr.nPort);
