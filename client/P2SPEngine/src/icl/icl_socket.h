...
public:
    ...
    int RecvBuffer(void *pBuffer, int nSize, CPeerAddress& RemoteAddr);
    int SendBuffer(void *pBuffer, int nSize, const CPeerAddress& RemoteAddr, int nSendTimes = 1);
};
...
public:
    // OnRecvDataEvent 的事件参数类型
    struct CRecvDataEventParam
    {
    public:
        void *pPacketBuffer;
        int nPacketSize;
        const CPeerAddress& RemoteAddr;
    public:
        CRecvDataEventParam(void *pPacketBufferA, int nPacketSizeA, const CPeerAddress& RemoteAddrA) :
            pPacketBuffer(pPacketBufferA), nPacketSize(nPacketSizeA), RemoteAddr(RemoteAddrA) {}
    };

    CEvent<CUdpServer, CRecvDataEventParam> OnRecvDataEvent;
public:
    CUdpServer();
    virtual ~CUdpServer();
    ...
    // 事件函数: 收到数据包
    virtual void OnRecvData(void *pPacketBuffer, int nPacketSize, const CPeerAddress& RemoteAddr)
    { OnRecvDataEvent.Invoke(*this, CRecvDataEventParam(pPacketBuffer, nPacketSize, RemoteAddr)); }
